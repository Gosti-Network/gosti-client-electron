...
class SprigganMinter():
	...
	async def __create_mint_spend_bundles(self):
		try:
			...
			if mintwallet == None:
				...
				response = await wallet_client.create_new_nft_wallet(self.mintingConfig["publisherDid"], name='Spriggan Mint')
				print(response)
				wallets = await wallet_client.get_wallets()
				for wallet in wallets:
			...
			with open(filename, 'w') as f:
				...
				royalty_percentage = int(float(self.mintingConfig["royaltyPercentage"])*100)
				...
				for i in range(0, mint_quantity):
					f.write(f"{data_hash},{data_uris[0]},{metadata_hash},{metadata_uris[0]},{license_hash},{license_uris[0]},{0},{0}\n")
			...
			with open(MINTING_DATA_ROOT + "minting_spend_bundles_output.pkl", "wb") as f:
	...
	async def __submit_spend_bundles(self):
		...
		try:
			...
			error = None
			spends = []
			with open(MINTING_DATA_ROOT + "minting_spend_bundles_output.pkl", "rb") as f:
