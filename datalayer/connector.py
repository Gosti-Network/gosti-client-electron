...
class DataLayerConnector:
    ...
    def create_datastore(self, fee):
        return asyncio.run(self.__create_datastore(fee))

    async def __create_datastore(self, fee):
        dl = await DataLayerRpcClient.create(
            "localhost",
            uint16(self.datalayer_rpc_port),
            DEFAULT_ROOT_PATH,
            self.chia_config,
        )
        store = await dl.create_data_store(fee=fee)
        print(f"datastore: {store}")
        dl.close()
        return store

    def publish_media(self, datastore_id, media, fee):
        return asyncio.run(self.__publish_media(datastore_id, media, fee))

    async def __publish_media(self, datastore_id, media, fee):
        try:
                ...
                if "productId" in gm and gm["productId"] == media["productId"]:
                    ...
                    update_status = await dl.update_data_store(
                        ...
                        changelist=[
                            {
                                "action": "delete",
                                "key": str.encode(media["productId"]).hex(),
                            },
                            {
                                "action": "insert",
                                "key": str.encode(media["productId"]).hex(),
                                "value": str.encode(g).hex(),
                            },
                        ],
                        fee=fee,
                    )
    ...
    async def __get_published_media(self, id):
        try:
            ...
            medias = await dl.get_keys_values(
                store_id=hexstr_to_bytes(id), root_hash=hexstr_to_bytes(root["hash"])
            )
            dl.close()
            decoded = []
